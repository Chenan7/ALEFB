...
class CA_Block(nn.Module):
    def __init__(self, channel, reduction=16):
        ...
        self.bagf = BAGF(channel, channel)
    ...
    def forward(self, x):
        ...
        x2=torch.cat((x_h, x_w), 3)
        x1= self.bagf(x_h,x_w)
        #print(x_h.shape,x_w.shape,x1.shape,x2.shape)
        _, _, _, current_width = x1.size()
...
class MBConvBlock(nn.Module):
    def __init__(self, block_args, global_params, add_ca=False):
        super().__init__()
...
class EfficientNetWithCA(nn.Module):
    def __init__(self, blocks_args=None, global_params=None):
        ...
        for i in range(len(self._blocks_args)):
            ...
            add_ca = False
            if is_last_block and i in [2, 3, 4]:  # 只在Block3, Block4和Block5后加CA
                add_ca = True
            ...
            self._blocks.append(MBConvBlock(self._blocks_args[i], self._global_params, add_ca=add_ca))
...
def efficientnet_argff(pretrained=False, progress=True, num_classes=10):
    ...
    model = EfficientNetWithCA(blocks_args, global_params)
    return model
